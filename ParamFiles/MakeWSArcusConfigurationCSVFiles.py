...
import arcusTrace.ParamFiles.arcus_params_wsdesign_rev1p0 as cfpar
...
def InitializeArcusChannel(facet_is):
    ...
    arcus_xous = dict(('OC1_XOU' + str(i),ArcComp.xou(i,float(cfpar.F), float(cfpar.xou_z0[i]), float(cfpar.xou_irs[i]),float(cfpar.xou_ors[i]),float(cfpar.xou_kinkr[i]),float(cfpar.xou_widths[i]),float(cfpar.xou_lengths[i]),float(cfpar.xou_lengths[i]),float(cfpar.xou_cangles[i]*pi/180),float(cfpar.pore_space),float(cfpar.plate_space))) for i in range(cfpar.N_xous))
    ...
    for i in range(cfpar.N_xous):
        ...
        arcus_xous['OC1_XOU' + str(i)].set_row(cfpar.arcus_xou_row_number[i])
...
arcus_xous,arcus_facets = InitializeArcusChannel(list(sorted_facets[:,0].astype(int)))
times.append(time.time())
...
def write_spos_to_bitbucket(arcus_xous,csv_fn = 'Arcus_SPO_XOU_Specs_Rev2p0_200424.csv'):
    sorted_xous = zip(*sorted(tuple([(arcus_xous[key].xou_num,arcus_xous[key]) for key in arcus_xous.keys()])))
    
    keys_to_track = ['xou_num','chan_id','MM_num','row_num','focal_length','z0',\
                    'inner_radius','outer_radius','kink_radius',\
                     'azwidth','clocking_angle','primary_length','secondary_length',\
                     'pore_space','plate_height','plate_coating']
    keys_units = ['-','-','-','-','mm','mm','mm','mm','mm','mm','radians','mm','mm','mm','mm','-']
    ...
    with open(csv_fn,'wb') as csvfile:
        ...
        csvfile.write('# ORIGFILE: arcusTrace.ParamFiles.arcus_params_wsdesign_rev1p0.py\n')
        csvfile.write('# Description: Specifications of SPOs from preliminary specifications for Arcus 2020 (E. Hertz, as of 20/04/24)' + '\n')
...
def write_facets_to_bitbucket(arcus_facets,csv_fn = 'Arcus_CATGrating_Facets_Specs_Rev2p0_200424.csv'):
    ...
    with open(csv_fn,'wb') as csvfile:
        ...
        csvfile.write('# Author: Casey DeRoo\n')
        csvfile.write('# ORIGFILE: arcusTrace.ParamFiles.arcus_params_wsdesign_rev1p0.py\n')
        csvfile.write('# Description: Specifications of CAT Grating Facets from preliminary specifications for Arcus 2020 (E. Hertz, as of 20/04/24)' + '\n')
