...
reflib_dir = 'C:\Users\Casey\Software\ReflectLib\'
...
energies = np.arange(100.,1500. + 2.5,2.5)
roughness = np.arange(0.0,0.1,0.1)#1.0 + 0.1,0.1)
max_step = 500
...
for rough in roughness:
    fn = 'SL_LSi02_LThick1p0_LRough' + "{:2.1f}".format(rough).replace('.','p') + '_SSi_SRough0p0_Pol1_CXRO.npy'
    Arcus_NoCoat.layer_roughness_nm = rough
    save_data = np.zeros((len(energies),len(angles),3))
    for i in range(len(angles)):
        save_data[:,i,:] = make_that_data_stack(Arcus_NoCoat,angles[i])
